...
async function inspectCanvas(page) {
  return page.evaluate(() => {
    ...
    let alpha = -1, width = 0, height = 0, glError = '';
    if (canvas) {
      width = canvas.width;
      ...
      } catch (err) { glError = String(err?.message || err); }
    }
    return { loadingExists: !!loading, loadingText, canvasExists: !!canvas, width, height, alpha, glError, pixiTest: window.__PIXITEST__ || null };
  });
}
...
for (const [name, browserType] of engines) {
  ...
  try {
    // A: raw PixiJS 8 texture upload/render, no Live2D engine involved.
    {
      ...
      if (consoleMessages.length) console.log(`[${name}][raw-pixi-texture] console:`, consoleMessages.join('\n'));
      if (pageErrors.length) console.log(`[${name}][raw-pixi-texture] pageerror:`, pageErrors.join('\n'));
      if (!ok) {
        failed = true;
    ...
    // B: same browser/GPU, Live2D engine rendering the same texture.
    {
      ...
      if (consoleMessages.length) console.log(`[${name}][live2d] console:`, consoleMessages.join('\n'));
      if (pageErrors.length) console.log(`[${name}][live2d] pageerror:`, pageErrors.join('\n'));
      if (!ok) {
        failed = true;
        console.error(`[${name}][live2d] FAILED: Live2D engine did not produce visible alpha pixels.`);
      } else {

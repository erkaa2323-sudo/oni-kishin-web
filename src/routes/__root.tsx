...
const isRecoverableClientLoadError = (error: Error) =>
  /Failed to fetch dynamically imported module|Importing a module script failed|Load failed|ChunkLoadError|error loading dynamically imported module/i.test(
  ...
  try {
    ...
    if ("caches" in window) {
      ...
      await Promise.allSettled(
        keys.filter((key) => key.startsWith("oni-")).map((key) => caches.delete(key)),
      );
    }
  } catch (error) {
    console.warn("ONI recovery cleanup failed", error);
  }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Түр алдаа гарлаа. Доорх алдааны кодыг ашиглан яг шалтгааныг засна.
        </p>
        <pre
          className="mt-4 max-h-40 overflow-auto whitespace-pre-wrap break-words rounded-md border border-border bg-black/30 p-3 text-left text-xs text-red-300"
          data-testid="oni-client-error"
        >
          {diagnostic}
        </pre>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "ONI NEXUS — Oni And Kishin" },
      {
        name: "description",
        content:
          "ONI NEXUS — Oni And Kishin-ийн iPhone standalone app, Shizuki AI, Meet, Crew, Garage болон push notification төв.",
      },
      { property: "og:site_name", content: "ONI NEXUS" },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "mobile-web-app-capable", content: "yes" },
      { name: "apple-mobile-web-app-capable", content: "yes" },
      { name: "apple-mobile-web-app-title", content: "ONI NEXUS" },
      { name: "apple-mobile-web-app-status-bar-style", content: "black-translucent" },
      { name: "application-name", content: "ONI NEXUS" },
    ],
    links: [
      { rel: "stylesheet", href: appCss },
      { rel: "stylesheet", href: mobileAppCss },
      { rel: "preconnect", href: "https://fonts.googleapis.com" },
      { rel: "preconnect", href: "https://fonts.gstatic.com", crossOrigin: "anonymous" },
      {
        rel: "stylesheet",
        href: "https://fonts.googleapis.com/css2?family=Oswald:wght@400;500;600;700&family=Rubik:wght@300;400;500;600&family=JetBrains+Mono:wght@400;500&display=swap",
      },

...
function json(data, status, origin, extra={}) {
  return new Response(JSON.stringify(data), {status, headers:{...cors(origin), ...extra}});
}
...
function rateLimit(key) {
  ...
  return row.count > MAX_REQUESTS
    ? Math.max(1, Math.ceil((WINDOW_MS - (now-row.start))/1000))
    : null;
}
...
export default {
  async fetch(req, env, ctx) {
    ...
    if (new URL(req.url).pathname !== AI_PATH)
    ...
    if (!origin)
      return json({ok:false,error:"Origin required"},403,responseOrigin);
    if (req.method !== "POST")
      return json({ok:false,error:"Method not allowed"},405,responseOrigin,{Allow:"POST, OPTIONS"});

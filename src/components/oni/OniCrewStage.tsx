...
export function OniCrewStage() {
  ...
  const visible = useMemo(
    () => (filter === "all" ? roster : roster.filter((member) => member.roleId === filter)),
    [roster, filter],
  );
  ...
  const selectFilter = (nextFilter: Filter) => {
    ...
    const next =
      nextFilter === "all" ? roster : roster.filter((member) => member.roleId === nextFilter);
      ...
      <main>
        <section className="crew-select" aria-labelledby="crew-title">
          ...
          <header className="crew-select__heading">
            ...
            <span>
              {loadState === "ready"
                ? `${String(visible.length).padStart(2, "0")} / ${String(roster.length).padStart(2, "0")}`
                : "— / —"}
            </span>
          </header>
          ...
          <nav className="crew-select__roles" aria-label="Үүргээр шүүх">
            {[{ id: "all" as const, label: "БҮГД" }, ...CREW_ROLES].map((role) => (
              <button
                key={role.id}
                type="button"
                aria-pressed={filter === role.id}
                onClick={() => selectFilter(role.id as Filter)}
              >
                {role.label}
              </button>
            ))}
          ...
          {loadState !== "ready" || !roster.length ? (
            <div className="crew-select__state">
              ...
              <p>
                {loadState === "loading"
                  ? "ONI архивтай холбогдож байна…"
                  : loadState === "error"
                    ? loadError
                    : "Идэвхтэй гишүүн бүртгэгдээгүй байна."}
              </p>
            </div>
          ) : active ? (
            <div className="crew-select__stage">
              <article key={`info-${active.id}`} className="crew-select__identity">
                <div className="crew-select__status">
                  <Radio />
                  {CREW_STATUS_LABEL[active.status]}
                </div>
                <p className="crew-select__role">
                  {CREW_ROLES.find((role) => role.id === active.roleId)?.label ?? active.roleId}
                </p>
              ...
              <aside className="crew-select__chapter" aria-label="Сонгосон гишүүний дугаар">
                ...
                <small>{active.roleId.toUpperCase()}</small>
              </aside>
            </div>
          ) : null}
          ...
          {loadState === "ready" && roster.length ? (
            <div className="crew-select__roster">
              <button type="button" onClick={() => step(-1)} aria-label="Өмнөх гишүүн">
                <ChevronLeft />
              </button>
              ...
              <button type="button" onClick={() => step(1)} aria-label="Дараагийн гишүүн">
                <ChevronRight />
              </button>
            </div>
          ) : null}

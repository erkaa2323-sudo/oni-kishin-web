...
import { OniCreatorStudio } from "./OniCreatorStudio";
...
export function OniAiChamber() {
  ...
  const audioRef = useRef<HTMLAudioElement | null>(null),
    track = tracks[trackIndex] ?? EMPTY_TRACK,
    hasTracks = tracks.length > 0,
    effectiveDuration = audioDuration > 0 ? audioDuration : track.duration;
  const goTo = (i: number) => {
  ...
  useEffect(() => {
    ...
    return () => window.clearInterval(id); // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const [convState, setConvState] = useState<OniState | null>(null),
    state = resolveState(convState, playing),
    visual = ONI_STATE_VISUALS[state],
  ...
  const composer = (
    ...
    >
      ...
      <button
        ...
        aria-label="Shizuki Creator Studio"
        className="grid h-11 w-11 shrink-0 place-items-center border border-crimson/45 bg-crimson/10 text-crimson clip-notch"
      >
      ...
      <button
        ...
        aria-label="Илгээх"
        disabled={thinking}
        className="grid h-11 w-11 shrink-0 place-items-center border border-crimson/50 bg-crimson/15 text-foreground clip-notch disabled:opacity-50"
      >
  ...
  const transport = (compact = false) => (
    <div className="flex items-center gap-2">
      <button
        ...
        aria-label="Өмнөх"
        disabled={!hasTracks}
        className="grid h-11 w-11 shrink-0 place-items-center border border-border text-foreground clip-notch disabled:opacity-40"
      >
      ...
      <button
        ...
        disabled={!hasTracks}
        className="grid h-11 w-11 shrink-0 place-items-center border border-crimson/55 bg-crimson/18 text-foreground clip-notch disabled:opacity-40"
      >
      ...
      {compact && (
        <button
          ...
          aria-label="Хөгжмийн тохиргоо"
          className="grid h-11 w-11 shrink-0 place-items-center border border-border text-muted-foreground clip-notch"
        >
  ...
  const progressBar = (
    <div
      ...
      aria-valuemin={0}
      aria-valuemax={Math.max(effectiveDuration, 1)}
  ...
  const queueList = !hasTracks ? (
    <div className="flex min-h-0 flex-1 items-center justify-center p-6">
      <p className="max-w-xs text-center text-xs leading-relaxed text-muted-foreground">
        Нийтлэгдсэн трэк одоогоор алга.
      </p>
    </div>
  ) : (
      ...
      {tracks.map((t, i) => (
        <li key={t.id} className="bg-ink/60">
          <button
            ...
            className={`flex min-h-[44px] w-full items-center gap-3 px-3 py-2.5 text-left ${i === trackIndex ? "bg-crimson/12" : ""}`}
          >
  ...
  const toggles = (
    <div className="flex items-center gap-2">
      <button
        ...
        aria-label="Холих"
        className={`grid h-11 w-11 place-items-center border clip-notch ${shuffle ? "border-crimson/60 text-crimson" : "border-border text-muted-foreground"}`}
      >
      ...
      <button
        ...
        aria-pressed={repeat}
        aria-label="Давтах"
        className={`grid h-11 w-11 place-items-center border clip-notch ${repeat ? "border-crimson/60 text-crimson" : "border-border text-muted-foreground"}`}
      >
      ...
      <button
        ...
        aria-pressed={muted}
        className="inline-flex min-h-[44px] items-center gap-2 border border-border px-3 text-[0.55rem] tracking-[0.22em] text-muted-foreground clip-notch"
      >
  ...
  const createButton = (
    <button
      type="button"
      onClick={() => setCreatorOpen(true)}
      className="inline-flex min-h-11 items-center gap-2 border border-crimson/55 bg-crimson/12 px-3 text-[.6rem] font-semibold tracking-[.16em] text-foreground clip-notch"
    >
  ...
  return (
    <div className="relative bg-ink">
      ...
      >
        ...
        <div className="relative z-10 flex min-h-0 flex-1 flex-col px-3 pb-2 pt-[4.25rem]">
          <section className="relative min-h-0 flex-[1.05] overflow-hidden border border-border bg-midnight/40 clip-notch">
            ...
            <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-2.5">
              <span className="min-w-0">
                ...
                <span className="hud-label block text-[0.5rem]">ONI SHIZUKI · CHAT · CREATE</span>
              </span>
              {createButton}
            </div>
              ...
              {ONI_SUGGESTIONS.map((s) => (
                <li key={s}>
                  <button
                    ...
                    onClick={() => send(s)}
                    disabled={thinking}
                    className="min-h-[38px] whitespace-nowrap border border-border px-3 text-[.6rem] text-muted-foreground clip-notch disabled:opacity-50"
                  >
                    {s}
                  </button>
                </li>
          ...
          <section className="mt-2 shrink-0 border border-border bg-midnight/60">
            ...
            <div className="flex items-center gap-2 p-2">
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[.8rem] text-foreground">{track.title}</span>
                <span className="hud-label block truncate text-[.5rem]">
                  {formatTime(position)} / {formatTime(effectiveDuration)} · {track.artist}
                </span>
              </span>
              {transport(true)}
            </div>
        ...
        <div
          className={`absolute inset-0 z-20 transition-opacity duration-300 ${drawer ? "opacity-100" : "pointer-events-none opacity-0"}`}
          aria-hidden={!drawer}
        >
          <div
      ...
      <main className="relative hidden lg:block">
        <section className="relative isolate overflow-hidden" aria-labelledby="oniai-title">
          ...
          <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[110rem] flex-col px-8 pb-10 pt-28">
            <header className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4">
              <div className="min-w-0">
                <span className="hud-label hud-rule block pl-11 text-crimson/85">
                  SECTOR 03 / ONI SHIZUKI · CHAT · CREATE
                </span>
            ...
            <div className="mt-8 grid flex-1 gap-px bg-border lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.3fr)_minmax(0,1fr)]">
              <div className="relative flex min-h-[26rem] flex-col justify-end overflow-hidden bg-midnight/50 p-6">
                ...
                <div className="relative">
                  ...
                  <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                    Кланы AI туслах, creative director, хөгжмийн систем нэг танхимд.
                  </p>
                  ...
                  {ONI_SUGGESTIONS.map((s) => (
                    <li key={s}>
                      <button
                        ...
                        onClick={() => send(s)}
                        disabled={thinking}
                        className="min-h-[44px] whitespace-nowrap border border-border px-3 text-[.65rem] text-muted-foreground clip-notch disabled:opacity-50"
                      >
                        {s}
                      </button>
                    </li>
              ...
              <div className="flex min-h-[26rem] flex-col bg-midnight/40 p-6">
                ...
                <div className="mt-4 border border-border bg-ink/70 p-4 clip-notch">
                  ...
                  <div className="mt-2 flex justify-between text-[.6rem] tracking-[.2em] text-muted-foreground">

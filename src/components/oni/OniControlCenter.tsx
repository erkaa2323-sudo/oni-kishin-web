        ...
        <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
          {filters.map((f) => (
            <button
              ...
              className={`min-h-[44px] shrink-0 border px-3.5 text-[0.6rem] tracking-[0.2em] transition-colors clip-notch ${filter === f.value ? "border-crimson/60 bg-crimson/15 text-foreground" : "border-border text-muted-foreground hover:text-foreground"}`}
            >
              {f.label}
            </button>
          ))}
      ...
      <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
        {tabs.map((t) => (
          <button
            ...
            className={`min-h-[44px] shrink-0 border px-3.5 text-[0.6rem] tracking-[0.2em] transition-colors clip-notch ${tab === t.value ? "border-crimson/60 bg-crimson/15 text-foreground" : "border-border text-muted-foreground hover:text-foreground"}`}
          >
            {t.label}
          </button>
        ))}
        ...
        {COMMAND_TIERS.map((t) => (
          <li key={t.id} className="bg-ink/60">
            <button
              ...
              className={`h-full w-full p-4 text-left transition-colors ${tier === t.id ? "bg-crimson/12" : "hover:bg-midnight/60"}`}
            >
              ...
              {ADMIN_MODULES.map((m) => (
                <li key={m.id} className="shrink-0 lg:bg-ink">
                  <button
                    ...
                    className={`flex min-h-[44px] w-full items-center gap-3 border px-3.5 text-left transition-colors clip-notch lg:border-0 lg:py-3 ${active === m.id ? "border-crimson/60 bg-crimson/15 text-foreground" : "border-border text-muted-foreground hover:text-foreground lg:hover:bg-midnight/60"}`}
                  >
          ...
          >
            ...
            {active === "members" ? (
              <CrudWorkspace
                ...
                fields={MEMBER_ADMIN_FIELDS}
                load={async () => {
                  const r = await getMembers();
                  if (r.status !== "ok") return r;
                  return { status: "ok", rows: r.rows.map(mapAdminMemberRow) };
                }}
            ...
            {active === "garage" ? (
              <CrudWorkspace
                ...
                fields={[
                  ...
                  { key: "owner_member_id", label: "ЭЗЭМШИГЧИЙН ONI ID" },
                  { key: "category", label: "АНГИЛАЛ" },
                  { key: "build", label: "БҮТЭЦ" },
                  { key: "drivetrain", label: "ХӨТЛӨГЧ (RWD/AWD/FWD)" },
                  { key: "horsepower", label: "ХҮЧ (HP)", type: "number" },
                  { key: "livery_theme", label: "LIVERY СЭДЭВ" },
                  { key: "awards", label: "ШАГНАЛУУД (таслалаар)" },
                  {
                    key: "featured",
                    label: "ОНЦЛОХ МАШИН",
                    options: [

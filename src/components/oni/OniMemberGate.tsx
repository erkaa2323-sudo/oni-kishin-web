...
export function OniMemberGate({ onAccount }: Props) {
  ...
  if (phase === "signed_in") {
    ...
    return (
      <section
        className="mt-6 border border-border bg-midnight/55 p-4"
        aria-label="Crew мэдээлэл холбох"
      >
        <div className="flex items-start justify-between gap-3">
          <p className="text-xs text-amber-300">
          ...
          <button
            type="button"
            aria-label="Өөр аккаунтаар нэвтрэх"
            disabled={busy}
            onClick={() => void leaveIncompleteAccount()}
            className="grid h-11 w-11 shrink-0 place-items-center border border-border text-muted-foreground disabled:opacity-50"
          >
            <LogOut className="h-4 w-4" />
          </button>
        </div>

...
type HostState = "idle" | "scheduled" | "starting" | "open" | "live" | "loading" | "registered" | "access" | "denied" | "full" | "closed";
type Props = { life: MeetLifecycle; registrationState: RegistrationState; accessReady?: boolean; notice?: string; nickname?: string; participants: number; capacity: number | null; countdownPhase?: CountdownPhase; countdownSeconds?: number };
type KeiMessage = { source?: string; type?: string };

const VENDOR_BASE = `${import.meta.env.BASE_URL.replace(/\/?$/, "/")}vendor/live2d`;
const RUNTIME_URLS = {
  pixi: [`${VENDOR_BASE}/pixi.min.js`, "https://cdn.jsdelivr.net/npm/pixi.js@6.5.10/dist/browser/pixi.min.js"],
  core: [`${VENDOR_BASE}/live2dcubismcore.js`, "https://cubism.live2d.com/sdk-web/cubismcore/live2dcubismcore.min.js"],
  bridge: [`${VENDOR_BASE}/cubism4.min.js`, "https://cdn.jsdelivr.net/npm/pixi-live2d-display@0.4.0/dist/cubism4.min.js"],
};
...
function resolveHostState(life: MeetLifecycle, registrationState: RegistrationState, accessReady: boolean): HostState {
...
function copy(state: HostState, phase: CountdownPhase, seconds: number, nickname?: string, notice?: string) {
...
export function KeiMeetHostHardened({ life, registrationState, accessReady = false, notice, nickname, participants, capacity, countdownPhase = "none", countdownSeconds = 0 }: Props) {
  ...
  const countdownActive = countdownPhase !== "none" && state !== "access" && state !== "loading" && state !== "denied";
...
addEventListener('beforeunload',function(){disposed=true;if(resizeRaf)cancelAnimationFrame(resizeRaf);if(idleTimer)clearInterval(idleTimer);if(observer)observer.disconnect();removeEventListener('resize',queueFit);try{if(model)model.destroy({children:true,texture:true,baseTexture:true})}catch(e){}try{if(app)app.destroy(true,{children:true,texture:true,baseTexture:true})}catch(e){}});
  ...
  useEffect(() => {
    const armWatchdog = () => {
      ...
      watchdogRef.current = window.setTimeout(() => setRuntime((value) => value === "ready" ? value : "failed"), 18000);
    };
    armWatchdog();
    const onMessage = (event: MessageEvent<KeiMessage>) => {
      if (event.source !== frameRef.current?.contentWindow || event.data?.source !== "oni-kei-meet") return;
      if (event.data.type === "ready") { retryRef.current = 0; if (watchdogRef.current) window.clearTimeout(watchdogRef.current); setRuntime("ready"); }
      if (event.data.type === "failed") setRuntime("failed");
    };
    window.addEventListener("message", onMessage);
    return () => { if (watchdogRef.current) window.clearTimeout(watchdogRef.current); window.removeEventListener("message", onMessage); };
  }, [frameKey]);

  useEffect(() => {
    if (runtime !== "failed" || retryRef.current >= 3) return;
    const delays = [1400, 3000, 6000];
    const retry = () => { retryRef.current += 1; setRuntime("loading"); setFrameKey((value) => value + 1); };
  ...
  useEffect(() => {
    ...
    frameRef.current?.contentWindow?.postMessage({ source: "oni-kei-meet-parent", type: "state", state: reactionState }, "*");
  }, [reactionState, runtime]);

  const interact = () => {
    if (runtime === "ready") frameRef.current?.contentWindow?.postMessage({ source: "oni-kei-meet-parent", type: "interact" }, "*");
  };

  return <div onPointerDown={interact} className={`relative h-full min-h-0 overflow-hidden border bg-black/55 shadow-[0_24px_80px_rgba(0,0,0,0.62)] clip-notch ${positive ? "border-emerald-400/25" : hot ? "border-crimson/35" : "border-white/10"}`}>
    ...
    <div className="pointer-events-none absolute left-3 top-3 z-20 sm:left-4 sm:top-4"><div className="text-[0.42rem] font-semibold tracking-[0.24em] text-white/35">ONI // MEET HOST</div><div className="mt-1 flex items-center gap-2"><span className="text-[0.64rem] font-semibold tracking-[0.26em] text-white/85 sm:text-[0.72rem]">KEI</span><span className="rounded-full border border-white/10 bg-black/35 px-2 py-1 text-[0.4rem] tracking-[0.17em] text-white/45">{label}</span></div></div>
    <div className="pointer-events-none absolute right-3 top-3 z-20 flex items-center gap-1.5 rounded-full border border-white/8 bg-black/30 px-2 py-1.5"><span className={`h-1.5 w-1.5 rounded-full ${runtime === "ready" ? "bg-emerald-300" : runtime === "failed" ? "bg-crimson" : "bg-white/30"}`} /><span className="text-[0.42rem] font-semibold tracking-[0.16em] text-white/45">{runtime === "ready" ? "LINKED" : runtime === "failed" ? "OFFLINE" : "SYNC"}</span></div>
    <iframe key={frameKey} ref={frameRef} title="Kei Live2D Meet host" srcDoc={srcDoc} sandbox="allow-scripts allow-same-origin" className="pointer-events-none absolute inset-0 h-full w-full border-0 bg-transparent" onLoad={() => setRuntime("loading")} />
    ...
    {runtime === "failed" ? <div className="pointer-events-none absolute inset-0 z-10 grid place-items-center text-[0.46rem] tracking-[0.22em] text-white/30">KEI VISUAL OFFLINE · RETRYING</div> : null}
    <div className="pointer-events-none absolute inset-x-2 bottom-2 z-20 rounded-xl border border-white/8 bg-black/58 px-3 py-2.5 backdrop-blur-md sm:inset-x-3 sm:bottom-3 sm:px-4 sm:py-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className={`mb-1 text-[0.4rem] font-semibold tracking-[0.2em] ${positive ? "text-emerald-300/75" : hot ? "text-crimson/80" : "text-white/30"}`}>{hot ? "LIVE HOST SIGNAL" : "HOST LINK STANDBY"}</div><p className="line-clamp-2 text-[0.58rem] leading-relaxed text-white/82 sm:text-[0.67rem]">{message}</p></div><div className="shrink-0 rounded-lg border border-white/8 bg-white/[0.025] px-2.5 py-2 text-right"><div className="font-mono text-[0.72rem] font-semibold tracking-[0.06em] text-white/85 sm:text-[0.8rem]">{participants}/{capacity ?? "∞"}</div><div className="mt-0.5 text-[0.34rem] tracking-[0.18em] text-white/28">RIDERS</div></div></div></div>
  </div>;
}

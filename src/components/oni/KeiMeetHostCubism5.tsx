...
export function KeiMeetHostCubism5({ life, registrationState, accessReady = false, notice, nickname, participants, capacity, countdownPhase = "none", countdownSeconds = 0 }: Props) {
  ...
  const mode = countdownActive ? countdownPhase === "ten" ? "T-10 MIN" : countdownPhase === "five" ? "T-5 MIN" : countdownPhase === "one" ? "T-1 MIN" : countdownPhase === "final10" ? `T-${String(Math.max(1, countdownSeconds)).padStart(2, "0")}` : "GO LIVE" : modeFor(state);
  ...
  return (
    <div className={`relative h-full min-h-[360px] overflow-hidden rounded-[28px] border bg-black/20 shadow-2xl transition-colors duration-700 ${positive ? "border-emerald-300/35 shadow-emerald-400/10" : hot ? "border-crimson/40 shadow-crimson/15" : "border-white/10 shadow-crimson/10"}`} onPointerDown={interact}>
      ...
      <div className="pointer-events-none absolute inset-x-5 top-4 z-20 flex items-center justify-between text-[0.58rem] font-semibold tracking-[0.2em] text-white/55">
        ...
        <span className={runtime === "ready" ? "text-emerald-300/80" : runtime === "failed" ? "text-crimson" : "text-white/40"}>{runtime === "ready" ? "ONLINE" : runtime === "failed" ? "OFFLINE" : "SYNC"}</span>
      </div>

      <iframe key={retryKey} ref={frameRef} title="Kei Cubism 5 Meet host" src="/kei-live2d-host.html" className="relative z-[1] block h-[360px] w-full border-0 bg-transparent sm:h-[430px] lg:h-[500px]" onLoad={() => setRuntime("loading")} />
      ...
      <div className="pointer-events-none absolute bottom-4 left-4 z-20 w-[28%] max-w-[10rem] text-left sm:left-5 sm:w-[30%]">
        <div className={`text-[0.38rem] font-semibold tracking-[0.14em] ${positive ? "text-emerald-300/75" : hot ? "text-crimson/80" : "text-white/35"}`}>{signal}</div>
        <p className="mt-1 line-clamp-4 text-[0.48rem] leading-relaxed text-white/68 sm:text-[0.56rem]">{copy}</p>
      </div>

      <div className="pointer-events-none absolute bottom-4 right-4 z-20 w-[22%] max-w-[7rem] text-right sm:right-5">
        <div className="font-mono text-[0.9rem] font-semibold tracking-[0.06em] text-white/90 sm:text-[1rem]">{participants}/{capacity ?? "∞"}</div>

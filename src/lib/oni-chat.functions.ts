/**
 * Guarded general-conversation path for ONI BRAIN.
 *
 * The existing Cloudflare ONI Worker owns ONI's model access, personality and
 * web-search capability. This server function deliberately does not fall back
 * to Vercel AI Gateway: doing so can create an unexpected billing dependency.
 * When the Worker is unavailable we return { ok: false } and oni-brain.ts
 * continues with its deterministic, public-data-only local handlers.
 *
 * Security boundary: this path never reads meet credentials, room ids,
 * passwords, applications, profiles, roles or audit logs.
 */
  ...
  try {
    const response = await fetch(ONI_WORKER_ENDPOINT, {
      ...
      headers: {
        ...
        // This is a server-to-server request; browser users cannot control it.
    ...
    console.warn("[oni-brain] worker request failed", {

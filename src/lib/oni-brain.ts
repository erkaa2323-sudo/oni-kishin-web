...
export type BrainReply = { text: string; state?: OniState; sources?: Array<{ url: string; title: string }> };
export type BrainTurn = { role: "user" | "oni"; text: string };

const NO_DATA = "Одоогоор бүртгэгдсэн мэдээлэл алга байна.";
const HELP = "Би клан хамаарах нийтийн мэдээллийг хариулна: гишүүд, гараж, хөгжим, уулзалтын цаг/багтаамж/бүртгэлийн төлөв, элсэлт. Хамгаалагдсан мэдээлэл (ROOM ID, нууц үг) би хэзээ ч дамжуулахгүй.";

type Intent = "greet" | "meet" | "members" | "garage" | "music" | "stats" | "join" | "identity" | "help";
...
async function meetAnswer(): Promise<BrainReply> {
  ...
  if (load.status === "error") return { text: "Уулзалтын мэдээлэл ачаалж чадсангүй. Дараа дахин оролдоно уу.", state: "concerned" };
  ...
  const close = s.registrationClosesAt ? ` Бүртгэл хаагдах: ${fmtDate(s.registrationClosesAt)} (${countdown(s.registrationClosesAt)}).` : "";
  return { text: `«${s.title}» — ${LIFECYCLE_LABEL[life]}. Эхлэх: ${fmtDate(s.scheduledAt)}${s.scheduledAt ? ` (${countdown(s.scheduledAt)})` : ""}. Бүртгэгдсэн: ${cap}.${close} Бүртгүүлэхийг УУЛЗАЛТ хэсгээс хийнэ.`, state: life === "open" ? "excited" : "serious" };
}
...
async function membersAnswer(): Promise<BrainReply> {
  ...
  const names = res.data.slice(0, 8).map((m) => m.cpmNickname).join(", ");
  const more = res.data.length > 8 ? ` (+${res.data.length - 8})` : "";
  return { text: `Идэвхтэй гишүүд: ${res.data.length}. ${names}${more}. Дэлгэрэнгүйг КРЮ хэсгээс үзнэ үү.`, state: "happy" };
}
...
async function garageAnswer(): Promise<BrainReply> {
  ...
  const list = res.data.slice(0, 6).map((v) => `${v.model}${v.ownerName ? ` — ${v.ownerName}` : ""}`).join("; ");
  return { text: `Нийтэлсэн машин: ${res.data.length}. ${list}. Бүрэн жагсаалт ГАРАЖ хэсэгт.`, state: "excited" };
}
...
async function musicAnswer(): Promise<BrainReply> {
  ...
  return { text: `Плейлистэд ${res.data.length} трек байна: ${res.data.slice(0, 5).map((t) => t.title).join(", ")}.`, state: "music" };
}
...
async function statsAnswer(): Promise<BrainReply> {
  ...
  const [m, g, t, meet] = await Promise.all([membersService.listPublic(), garageService.listPublished(), musicService.listPublished(), fetchActiveMeet()]);
  const n = (r: { ok: boolean; data?: unknown[] }) => (r.ok && r.data ? r.data.length : "—");
  const meetLine = meet.status === "ok" && meet.session ? `Идэвхтэй уулзалт: «${meet.session.title}» (${LIFECYCLE_LABEL[deriveLifecycle(meet.session)]}).` : "Идэвхтэй уулзалт: алга.";
  return { text: `ONI & KISHIN төлөв — Гишүүд: ${n(m)} · Машин: ${n(g)} · Трек: ${n(t)}. ${meetLine}`, state: "serious" };
}
...
function classify(n: NormalizedInput): Intent | null {
  ...
  if (hasStem(n, "статистик", "тоо", "төлөв", "стат", "статус", "хэдэн", "хураангуй", "танилцуул", "клан", "они анд кишин")) return "stats";
...
function isFollowUp(n: NormalizedInput): boolean {
  ...
  return hasStem(n, "тэр", "тэд", "тэгээд", "тэгэд", "дахиад", "дэлгэрэнгүй", "дэлгэрэнгуй", "цааш", "өөр", "яг", "хэд", "хэн бэ", "жагса", "нэр");
}
...
async function generalFallback(raw: string, history: BrainTurn[]): Promise<BrainReply | null> {
  try {
    ...
    const [membersLoad, garageLoad, musicLoad, meetLoad] = await Promise.allSettled([
      membersService.listPublic(), garageService.listPublished(), musicService.listPublished(), fetchActiveMeet(),
    ]);
    ...
    const publicContext = JSON.stringify({
      ...
      music: music?.ok ? music.data.map((t) => ({ title: t.title, artist: t.artist ?? null })) : "unavailable",
      meet: meet?.status === "ok" && meet.session ? { title: meet.session.title, scheduledAt: meet.session.scheduledAt, registrationClosesAt: meet.session.registrationClosesAt, capacity: meet.session.capacity, registered: meet.session.registered, lifecycle: deriveLifecycle(meet.session) } : null,
    }).slice(0, 6000);
    const turns = [...history.slice(-10), { role: "user" as const, text: raw }]
      .filter((t) => t.text.trim().length > 0)
      .map((t) => ({ role: (t.role === "user" ? "user" : "assistant") as "user" | "assistant", content: t.text.slice(0, 1200) }));
...
export async function answerOni(input: string, history: BrainTurn[] = []): Promise<BrainReply> {
  ...
  if (isCredentialRequest(n.raw) || isCredentialRequest(n.haystack)) return { text: CREDENTIAL_REFUSAL, state: "serious" };
  ...
  return { text: `Үүнд яг таарах баталгаатай өгөгдөл надад алга байна. ${HELP}`, state: "concerned" };
}

...
export function getServiceStatuses(): ServiceStatus[] {
  return [
    ...
    },
    {
      ...
      note: "Oni Shizuki админ командын хөдөлгүүр идэвхтэй.",
    },
    {
      key: "meet",
      label: "УУЛЗАЛТ",
      code: "MEET SERVICE",
      state: "connected",
      note: "Firestore уулзалтын lifecycle, бүртгэл болон хамгаалагдсан ROOM мэдээллийн үйлчилгээ идэвхтэй.",
    },
    {
      key: "storage",
      label: "ФАЙЛ САН",
...
export async function getRegistrations(
  ...
  return toResult(
    ...
    (r) => ({ id: r.id, cpmNickname: r.cpmNickname, cpmId: r.cpmId, createdAt: r.createdAt ?? "" }),
  );
}
...
export async function dispatchAdminAction(
  ...
  const needsTarget = !(
    ["member.create", "vehicle.create", "track.create", "meet.create"] as string[]
  ).includes(kind);
  if (needsTarget && !targetId) return { ok: false, error: "Зорилтот бичлэг сонгогдоогүй байна." };
  ...
  if (kind === "meet.create" && String(data["status"] ?? "draft") !== "draft") {
    const push = await announceNexusMeet(
      typeof data["title"] === "string" ? data["title"] : "ONI MEET",
    ).catch(() => null);
    if (!push?.ok)
      console.warn(
        "[oni-nexus] meet created but push was not delivered",
        push && "message" in push ? push.message : "unknown",
      );
  }
...
export async function submitCommand(input: string, tier: CommandTier): Promise<CommandResponse> {
  ...
  if (hit("анкет", "application", "хүсэлт")) {
    ...
    const rationale = `Нийт анкет: ${r.rows.length} · Хүлээгдэж буй: ${pending.length} · Зөвшөөрсөн: ${r.rows.filter((a) => a.state === "accepted").length} · Татгалзсан: ${r.rows.filter((a) => a.state === "rejected").length}.`;
  ...
  if (hit("уулзалт", "meet")) {
    ...
    const rationale =
      live.length === 0
        ...
        : `Идэвхтэй/төлөвлөгдсөн уулзалт: ${live.map((m) => `«${m.title}» (${m.status})`).join(", ")}. Нийт: ${r.rows.length}.`;
    const actions: ProposedAction[] =
      tier === "read"
        ? []
        : [
  ...
  if (hit("гишүү", "member", "крю")) {
    ...
    const rationale = `Гишүүд: нийт ${r.rows.length} · идэвхтэй ${active} · архивлагдсан ${r.rows.filter((m) => m.status === "archived").length}.`;
    const actions: ProposedAction[] =
      tier === "read"
        ? []
        : [
  ...
  if (hit("гараж", "garage", "машин")) {
    ...
    if (r.status !== "ok") return { status: "error", message: r.reason };
    const rationale = `Машин: нийт ${r.rows.length} · нийтэлсэн ${r.rows.filter((v) => v.status === "published").length} · ноорог ${r.rows.filter((v) => v.status === "draft").length}.`;
    return {
      status: "plan",
  ...
  if (hit("хөгжим", "music", "трек")) {
    ...
    if (r.status !== "ok") return { status: "error", message: r.reason };
    const rationale = `Трек: нийт ${r.rows.length} · нийтэлсэн ${r.rows.filter((t) => t.status === "published").length}.`;
    return {
      status: "plan",
      plan: { id, tier, rationale, actions: [], requiresConfirmation: false },
    };

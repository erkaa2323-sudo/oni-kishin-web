...
export function getServiceStatuses(): ServiceStatus[] { return [
  {key:"backend",label:"СЕРВЕР",code:"BACKEND",state:ADMIN_BACKEND_CONNECTED?"connected":"not_connected",note:"Firebase үйлчилгээ идэвхтэй."},
  {key:"database",label:"ӨГӨГДЛИЙН САН",code:"DATABASE",state:ADMIN_BACKEND_CONNECTED?"connected":"not_connected",note:"Firestore мэдээллийн сан холбогдсон."},
  {key:"auth",label:"НЭВТРЭЛТ",code:"AUTH",state:ADMIN_AUTH_CONNECTED?"connected":"not_connected",note:"Нэвтрэлтийн үйлчилгээ идэвхтэй."},
  {key:"ai",label:"ОНИ ШИЗҮКИ",code:"AI ENGINE",state:ADMIN_AI_CONNECTED?"connected":"not_connected",note:"Oni Shizuki админ командын хөдөлгүүр идэвхтэй."},
  {key:"meet",label:"УУЛЗАЛТ",code:"MEET SERVICE",state:"connected",note:"Firestore уулзалтын lifecycle, бүртгэл болон хамгаалагдсан ROOM мэдээллийн үйлчилгээ идэвхтэй."},
...
const ACTION_PERMISSION:Record<AdminActionKind,AdminPermission>={"member.create":"members.write","member.update":"members.write","member.archive":"members.write","member.delete":"members.write","member_account.approve":"members.write","member_account.reject":"members.write","vehicle.create":"garage.write","vehicle.update":"garage.write","vehicle.archive":"garage.write","vehicle.delete":"garage.write","application.accept":"applications.review","application.reject":"applications.review","application.promote":"members.write","meet.create":"meet.control","meet.update":"meet.control","meet.start":"meet.control","meet.end":"meet.control","meet.close":"meet.control","meet.rotate_credentials":"meet.control","meet.registration_remove":"meet.control","track.create":"music.write","track.update":"music.write","track.delete":"music.write","prompt.update":"music.write"};
const NOT_IN_PHASE="Энэ үйлдэл дараагийн үе шатанд идэвхжинэ.";
function severityFor(kind:AdminActionKind):"info"|"warning"|"critical"{const risk=ACTION_RISK[kind];return risk==="high"?"critical":risk==="medium"?"warning":"info"}
export async function dispatchAdminAction(request:AdminActionRequest,actor:AdminActor|null):Promise<AdminActionResult>{
 const{kind,targetId,payload}=request;if(!actor)return{ok:false,error:"Нэвтрэлт баталгаажаагүй байна."};const needed=ACTION_PERMISSION[kind];if(!ROLE_PERMISSIONS[actor.role].includes(needed)){await recordAuditEvent({actorId:actor.uid,actorRole:actor.role,action:kind,target:targetId??undefined,severity:severityFor(kind),result:"denied",detail:"insufficient_permission"});return{ok:false,error:"Энэ үйлдэлд эрх хүрэхгүй байна."}}
 if(kind==="application.accept"&&!ROLE_PERMISSIONS[actor.role].includes("members.write"))return{ok:false,error:"Анкет батлахад гишүүн үүсгэх эрх шаардлагатай."};
 const{applicationsService,garageService,membersService,meetService,musicService}=await import("@/services/domains");const needsTarget=!(["member.create","vehicle.create","track.create","meet.create"] as string[]).includes(kind);if(needsTarget&&!targetId)return{ok:false,error:"Зорилтот бичлэг сонгогдоогүй байна."};const data=(payload??{}) as Record<string,unknown>;let res:{ok:true;data:unknown}|{ok:false;error:{message:string}};
 switch(kind){
 case"member.create":res=await membersService.create({...data,created_by:actor.uid,updated_by:actor.uid});break;case"member.update":res=await membersService.update(targetId!,{...data,updated_by:actor.uid});break;case"member.archive":res=await membersService.update(targetId!,{status:"archived",updated_by:actor.uid});break;case"member.delete":res=await membersService.remove(targetId!);break;
 case"member_account.approve":await reviewMemberAccount(targetId!,"approved");res={ok:true,data:{id:targetId!}};break;case"member_account.reject":await reviewMemberAccount(targetId!,"rejected");res={ok:true,data:{id:targetId!}};break;
 case"vehicle.create":res=await garageService.create({...data,created_by:actor.uid,updated_by:actor.uid});break;case"vehicle.update":res=await garageService.update(targetId!,{...data,updated_by:actor.uid});break;case"vehicle.archive":res=await garageService.update(targetId!,{status:"archived",updated_by:actor.uid});break;case"vehicle.delete":res=await garageService.remove(targetId!);break;
 case"application.accept":{const nickname=typeof data["cpm_nickname"]==="string"?data["cpm_nickname"]:"";const cpmId=typeof data["cpm_id"]==="string"?data["cpm_id"]:"";if(!nickname||!cpmId)return{ok:false,error:"Анкетын мэдээлэл дутуу тул батлах боломжгүй."};res=await applicationsService.acceptAndPromote(targetId!,{cpmNickname:nickname,cpmId},actor.uid);break}case"application.reject":res=await applicationsService.review(targetId!,"rejected",actor.uid);break;
 case"application.promote":{const nickname=typeof data["cpm_nickname"]==="string"?data["cpm_nickname"]:"";const cpmId=typeof data["cpm_id"]==="string"?data["cpm_id"]:"";if(!nickname||!cpmId)return{ok:false,error:"Анкетын мэдээлэл дутуу тул гишүүн үүсгэх боломжгүй."};res=await membersService.create({cpm_nickname:nickname,cpm_id:cpmId,status:"active",joined_at:new Date().toISOString(),created_by:actor.uid,updated_by:actor.uid});break}
 case"meet.create":res=await meetService.create({title:data["title"]??"",scheduled_at:data["scheduled_at"]??null,ends_at:data["ends_at"]??null,registration_closes_at:data["registration_closes_at"]??null,capacity:data["capacity"]??null,status:data["status"]??"draft",created_by:actor.uid,updated_by:actor.uid});break;case"meet.update":res=await meetService.update(targetId!,{title:data["title"]??"",scheduled_at:data["scheduled_at"]??null,ends_at:data["ends_at"]??null,registration_closes_at:data["registration_closes_at"]??null,capacity:data["capacity"]??null,updated_by:actor.uid});break;case"meet.start":res=await meetService.setLifecycle(targetId!,"live");break;case"meet.end":res=await meetService.setLifecycle(targetId!,"ended");break;case"meet.close":res=await meetService.setLifecycle(targetId!,"closed");break;
...
export type CommandTier="read"|"prepare"|"execute";export const COMMAND_TIERS:{id:CommandTier;label:string;code:string;desc:string}[]=[{id:"read",label:"УНШИХ",code:"READ",desc:"Зөвхөн хайх, шалгах, хураангуйлах. Өөрчлөлт хийхгүй."},{id:"prepare",label:"БЭЛТГЭХ",code:"PREPARE",desc:"Санал болгох үйлдлийн төлөвлөгөө боловсруулж, хянуулна."},{id:"execute",label:"ГҮЙЦЭТГЭХ",code:"EXECUTE",desc:"Эмзэг үйлдлийг зөвхөн админ баталгаажуулсны дараа гүйцэтгэнэ."}];
export type ProposedAction={kind:AdminActionKind;summary:string;risk:RiskLevel;missingParams:string[]};export type CommandPlan={id:string;tier:CommandTier;rationale:string;actions:ProposedAction[];requiresConfirmation:boolean};export type CommandResponse={status:"offline";message:string}|{status:"plan";plan:CommandPlan}|{status:"error";message:string};
export const COMMAND_SUGGESTIONS:{tier:CommandTier;text:string}[]=[{tier:"read",text:"Хүлээгдэж буй анкетуудыг шалга"},{tier:"read",text:"Идэвхтэй уулзалт байгаа эсэхийг шалга"},{tier:"prepare",text:"Маргаашийн уулзалт үүсгэх төлөвлөгөө бэлтгэ"},{tier:"prepare",text:"Идэвхгүй гишүүдийг архивлах саналыг бэлтгэ"}];
export async function submitCommand(input:string,tier:CommandTier):Promise<CommandResponse>{const raw=input.trim();if(!raw)return{status:"error",message:"Команд хоосон байна."};const q=raw.toLowerCase();if(/(pass|нууц\s*үг|password|room\s*id|өрөө.*(id|код))/i.test(raw))return{status:"error",message:"Уулзалтын ROOM ID / нууц үгийг командын самбараар харуулахгүй. УУЛЗАЛТ модулиас эрх шалгасны дараа удирдана."};const id=`cmd${Date.now()}`;const hit=(...w:string[])=>w.some(x=>q.includes(x));
 if(hit("анкет","application","хүсэлт")){const r=await getApplications();if(r.status!=="ok")return{status:"error",message:r.reason};const pending=r.rows.filter(a=>a.state==="pending");const rationale=`Нийт анкет: ${r.rows.length} · Хүлээгдэж буй: ${pending.length} · Зөвшөөрсөн: ${r.rows.filter(a=>a.state==="accepted").length} · Татгалзсан: ${r.rows.filter(a=>a.state==="rejected").length}.`;const actions:ProposedAction[]=tier==="read"||pending.length===0?[]:[{kind:"application.accept",summary:`Хүлээгдэж буй анкет: ${pending[0]!.cpmNickname} — зөвшөөрөх санал.`,risk:ACTION_RISK["application.accept"]??"low",missingParams:pending.length>1?["аль анкетыг сонгохыг тодруулна уу"]:[]}];return{status:"plan",plan:{id,tier,rationale,actions,requiresConfirmation:actions.length>0}}}
 if(hit("уулзалт","meet")){const r=await getMeets();if(r.status!=="ok")return{status:"error",message:r.reason};const live=r.rows.filter(m=>m.status==="live"||m.status==="scheduled");const rationale=live.length===0?`Идэвхтэй уулзалт алга. Нийт бүртгэл: ${r.rows.length}.`:`Идэвхтэй/төлөвлөгдсөн уулзалт: ${live.map(m=>`«${m.title}» (${m.status})`).join(", ")}. Нийт: ${r.rows.length}.`;const actions:ProposedAction[]=tier==="read"?[]:[{kind:"meet.create",summary:"Шинэ уулзалт үүсгэх — гарчиг, эхлэх цаг, багтаамж шаардлагатай.",risk:ACTION_RISK["meet.create"]??"low",missingParams:["гарчиг","эхлэх цаг","багтаамж"]}];return{status:"plan",plan:{id,tier,rationale,actions,requiresConfirmation:actions.length>0}}}
 if(hit("гишүү","member","крю")){const r=await getMembers();if(r.status!=="ok")return{status:"error",message:r.reason};const active=r.rows.filter(m=>m.status==="active").length;const rationale=`Гишүүд: нийт ${r.rows.length} · идэвхтэй ${active} · архивлагдсан ${r.rows.filter(m=>m.status==="archived").length}.`;const actions:ProposedAction[]=tier==="read"?[]:[{kind:"member.archive",summary:"Идэвхгүй гишүүнийг архивлах — аль гишүүнийг архивлахыг заана уу.",risk:ACTION_RISK["member.archive"]??"low",missingParams:["гишүүний нэр эсвэл ID"]}];return{status:"plan",plan:{id,tier,rationale,actions,requiresConfirmation:actions.length>0}}}
 if(hit("гараж","garage","машин")){const r=await getVehicles();if(r.status!=="ok")return{status:"error",message:r.reason};const rationale=`Машин: нийт ${r.rows.length} · нийтэлсэн ${r.rows.filter(v=>v.status==="published").length} · ноорог ${r.rows.filter(v=>v.status==="draft").length}.`;return{status:"plan",plan:{id,tier,rationale,actions:[],requiresConfirmation:false}}}
 if(hit("хөгжим","music","трек")){const r=await getTracks();if(r.status!=="ok")return{status:"error",message:r.reason};const rationale=`Трек: нийт ${r.rows.length} · нийтэлсэн ${r.rows.filter(t=>t.status==="published").length}.`;return{status:"plan",plan:{id,tier,rationale,actions:[],requiresConfirmation:false}}}
 if(hit("төлөв","статус","status","хураангуй","overview")){const[m,v,a,t]=await Promise.all([getMembers(),getVehicles(),getApplications(),getTracks()]);const c=<T>(r:DataResult<T>)=>r.status==="ok"?String(r.rows.length):"—";return{status:"plan",plan:{id,tier,rationale:`Гишүүд: ${c(m)} · Машин: ${c(v)} · Анкет: ${c(a)} · Трек: ${c(t)}.`,actions:[],requiresConfirmation:false}}}
 return{status:"error",message:"Командыг таньсангүй. Дэмжигдсэн: анкет, гишүүд, гараж, хөгжим, уулзалт, ерөнхий төлөв."};}
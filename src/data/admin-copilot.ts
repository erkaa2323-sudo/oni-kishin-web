...
export type CopilotActionKind =
  AdminActionKind | "gallery.create" | "gallery.update" | "gallery.delete" | "meet.delete";
...
function targetHint(raw: string): string {
  ...
  return raw
    .replace(
      /(устга|устгах|хас|хасах|archive|архив|approve|зөвшөөр|reject|татгалз|эхлүүл|дуусга|хаа|start|end|close|update|зас|өөрчил)/giu,
      " ",
    )
    .replace(
      /(гишүүн|member|crew|крю|машин|гараж|garage|анкет|application|хүсэлт|meet|уулзалт|gallery|галерей|трек|track|music|хөгжим)/giu,
      " ",
    )
    .replace(/\s+/g, " ")
    .trim();
...
async function findMember(raw: string) {
  ...
  return (
    data.rows.find(
      (m) => same(m.id, hint) || same(m.cpmId, hint) || contains(m.cpmNickname, hint),
    ) ?? null
  );
}
...
async function findVehicle(raw: string) {
  ...
  if (!hint) return data.rows.length === 1 ? data.rows[0]! : null;
  return (
    data.rows.find((v) => same(v.id, hint) || contains(v.model, hint) || contains(v.owner, hint)) ??
    null
  );
}
...
async function findApplication(raw: string) {
  ...
  return (
    data.rows.find(
      (a) => same(a.id, hint) || same(a.cpmId, hint) || contains(a.cpmNickname, hint),
    ) ?? null
  );
}
...
async function findMeet(raw: string) {
  ...
  return (
    data.rows.find((m) => m.status === "live") ??
    data.rows.find((m) => m.status === "scheduled") ??
    null
  );
}
...
async function findTrack(raw: string) {
  ...
  return (
    data.rows.find(
      (t) => same(t.id, hint) || contains(t.title, hint) || contains(t.artist, hint),
    ) ?? null
  );
}
...
async function findGallery(raw: string) {
  ...
  return (
    data.rows.find((g) => same(g.id, hint) || contains(g.title, hint) || contains(g.owner, hint)) ??
    null
  );
}
...
async function readSummary(q: string): Promise<CopilotReply | null> {
  if (has(q, "ерөнхий", "overview", "хураангуй", "статус", "төлөв")) {
    ...
    const count = (r: { status: string; rows?: unknown[] }) =>
      r.status === "ok" ? (r.rows?.length ?? 0) : "—";
    return {
  ...
  if (has(q, "анкет", "application", "хүсэлт")) {
    ...
    return {
      text: `Нийт ${r.rows.length} хүсэлт байна. Хүлээгдэж буй ${pending.length}. ${
        pending
          .slice(0, 5)
          .map((a) => a.cpmNickname)
          .join(", ") || "Pending хүсэлт алга."
      }`,
  ...
  if (has(q, "гишүүн", "member", "crew", "крю")) {
    ...
    return {
      text: `Гишүүд нийт ${r.rows.length}. Идэвхтэй ${active.length}. ${active
        .slice(0, 8)
        .map((m) => m.cpmNickname)
        .join(", ")}.`,
      state: "happy",
  ...
  if (has(q, "гараж", "garage", "машин")) {
    ...
    if (r.status !== "ok") return { text: r.reason, state: "concerned" };
    return {
      text: `Гаражид ${r.rows.length} машин байна. ${r.rows
        .slice(0, 6)
        .map((v) => `${v.model} — ${v.owner}`)
        .join("; ")}.`,
      state: "excited",
    };
  }
  ...
  if (has(q, "галерей", "gallery", "зураг")) {
    ...
    if (r.status !== "ok") return { text: r.reason, state: "concerned" };
    return {
      text: `Галерейд ${r.rows.length} зураг байна. ${r.rows
        .slice(0, 6)
        .map((g) => g.title)
        .join(", ")}.`,
      state: "happy",
  ...
  if (has(q, "уулзалт", "meet")) {
    ...
    if (r.status !== "ok") return { text: r.reason, state: "concerned" };
    const current =
      r.rows.find((m) => m.status === "live") ?? r.rows.find((m) => m.status === "scheduled");
    if (!current)
      return {
        text: `Нийт ${r.rows.length} уулзалтын бүртгэл байна. Одоо идэвхтэй уулзалт алга.`,
        state: "idle",
      };
    const regs = await getRegistrations(current.id);
    const regCount = regs.status === "ok" ? regs.rows.length : "—";
    return {
      text: `«${current.title}» — ${current.status}. Багтаамж ${current.capacity || "—"}, бүртгүүлсэн ${regCount}.`,
      state: current.status === "live" ? "excited" : "serious",
    };
  }
  ...
  if (has(q, "хөгжим", "music", "трек", "track")) {
    ...
    if (r.status !== "ok") return { text: r.reason, state: "concerned" };
    return {
      text: `Хөгжимд ${r.rows.length} трек байна. ${r.rows
        .slice(0, 6)
        .map((t) => t.title)
        .join(", ")}.`,
      state: "music",
  ...
  if (has(q, "audit", "бүртгэл", "хэн юу", "үйлдлийн түүх")) {
    ...
    if (r.status !== "ok") return { text: r.reason, state: "concerned" };
    return {
      text: `Сүүлийн ${Math.min(r.rows.length, 6)} үйлдэл: ${r.rows
        .slice(0, 6)
        .map((e) => `${e.action} (${e.severity})`)
        .join("; ")}.`,
      state: "serious",
    };
  }
...
export async function runAdminCopilotCommand(
  ...
  if (has(q, "анкет", "application", "хүсэлт")) {
    ...
    if (!item)
      return {
        text: "Яг аль хүсэлтийг сонгохыг нэр эсвэл CPM ID-аар хэлнэ үү.",
        state: "listening",
      };
    if (has(q, "approve", "зөвшөөр", "батал")) {
      return action(
        "application.accept",
        `${item.cpmNickname} хүсэлтийг зөвшөөрч гишүүн болгоно.`,
        item.id,
        { cpm_nickname: item.cpmNickname, cpm_id: item.cpmId },
      );
    }
    if (has(q, "reject", "татгалз"))
      return action("application.reject", `${item.cpmNickname} хүсэлтийг татгалзана.`, item.id);
  }
  ...
  if (has(q, "гишүүн", "member", "crew", "крю")) {
    if (has(q, "нэм", "үүсгэ", "create", "add")) {
      ...
      if (!name || !idMatch)
        return {
          text: "Гишүүн нэмэхдээ нэрийг ишлэлд, CPM ID-г хамт өгнө үү. Жишээ: гишүүн «Hugo» нэм CPM ID WS123456",
          state: "listening",
        };
      return action("member.create", `${name} гишүүнийг нэмнэ.`, undefined, {
        cpm_nickname: name,
        cpm_id: idMatch,
        status: "active",
      });
    ...
    if (!item)
      return {
        text: "Яг аль гишүүнийг сонгохыг nickname эсвэл CPM ID-аар хэлнэ үү.",
        state: "listening",
      };
    if (has(q, "устга", "delete", "хас"))
      return action("member.delete", `${item.cpmNickname} гишүүнийг бүр мөсөн устгана.`, item.id);
    if (has(q, "archive", "архив", "идэвхгүй"))
      return action("member.archive", `${item.cpmNickname} гишүүнийг архивлана.`, item.id);
    if (has(q, "role", "үүрэг", "төрөл", "drone", "дрон", "special")) {
      const role =
        raw.match(/(?:role|үүрэг|төрөл)\s*[:=]?\s*([\p{L}0-9 _-]+)/iu)?.[1]?.trim() ||
        (has(q, "drone", "дрон") ? "Drone" : has(q, "special") ? "Special member" : "");
      if (!role) return { text: "Шинэ role-ийг хэлнэ үү.", state: "listening" };
      return action(
        "member.update",
        `${item.cpmNickname} гишүүний role-г ${role} болгоно.`,
        item.id,
        { role },
      );
    }
  ...
  if (has(q, "гараж", "garage", "машин")) {
    if (has(q, "нэм", "үүсгэ", "create", "add")) {
      ...
      if (!model)
        return {
          text: "Машины model/name-ийг ишлэлд өгнө үү. Жишээ: гаражид «Nissan Silvia S15» нэм owner Kitsune",
          state: "listening",
        };
      return action("vehicle.create", `${model} машиныг гаражид нэмнэ.`, undefined, {
        model,
        owner_name: owner,
        status: "published",
      });
    ...
    if (!item)
      return {
        text: "Яг аль машиныг сонгохыг model эсвэл owner-оор хэлнэ үү.",
        state: "listening",
      };
    if (has(q, "устга", "delete", "хас"))
      return action("vehicle.delete", `${item.model} машиныг гаражаас устгана.`, item.id);
    if (has(q, "archive", "архив"))
      return action("vehicle.archive", `${item.model} машиныг архивлана.`, item.id);
    if (has(q, "publish", "нийтэл"))
      return action("vehicle.update", `${item.model} машиныг published болгоно.`, item.id, {
        status: "published",
      });
  }
  ...
  if (has(q, "галерей", "gallery", "зураг")) {
    if (has(q, "нэм", "үүсгэ", "create", "add")) {
      ...
      const category = has(q, "drift", "дрифт")
        ? "drift"
        : has(q, "anime", "аним")
          ? "anime"
          : has(q, "clean", "цэвэр")
      ...
      if (!url)
        return {
          text: "Gallery-д нэмэх HTTPS зурагны холбоосыг команддаа оруулна уу.",
          state: "listening",
        };
      return action(
        "gallery.create",
        `«${title}» зургийг галерейд ${category} ангиллаар нэмнэ.`,
        undefined,
        { title, owner: "Oni And Kishin", category, build: "", image: url },
      );
    }
    const item = await findGallery(raw);
    if (!item)
      return { text: "Яг аль gallery item-ийг сонгохыг title-аар хэлнэ үү.", state: "listening" };
    if (has(q, "устга", "delete", "хас"))
      return action("gallery.delete", `«${item.title}» зургийг галерейгаас устгана.`, item.id);
    if (has(q, "category", "ангилал", "drift", "дрифт", "anime", "аним", "clean", "цэвэр")) {
      ...
      return action(
        "gallery.update",
        `«${item.title}» зургийн ангиллыг ${category} болгоно.`,
        item.id,
        { category },
      );
    }
  ...
  if (has(q, "уулзалт", "meet")) {
    if (has(q, "нэм", "үүсгэ", "create")) {
      ...
      return action(
        "meet.create",
        `«${title}» уулзалтыг ${capacity} хүний багтаамжтай үүсгэнэ. Цагийг дараа нь Meet module эсвэл update командаар тохируулж болно.`,
        undefined,
        { title, capacity, status: "draft" },
      );
    }
    const item = await findMeet(raw);
    if (!item) return { text: "Удирдах уулзалт олдсонгүй.", state: "concerned" };
    if (has(q, "эхлүүл", "start"))
      return action("meet.start", `«${item.title}» уулзалтыг LIVE эхлүүлнэ.`, item.id);
    if (has(q, "дуусга", "end"))
      return action("meet.end", `«${item.title}» уулзалтыг дуусгана.`, item.id);
    if (has(q, "хаа", "close"))
      return action("meet.close", `«${item.title}» уулзалтыг хаана.`, item.id);
    if (has(q, "устга", "delete"))
      return action(
        "meet.delete",
        `«${item.title}» уулзалтын бичлэгийг бүр мөсөн устгана.`,
        item.id,
      );
  }
  ...
  if (has(q, "хөгжим", "music", "трек", "track")) {
    ...
    if (item && has(q, "устга", "delete", "хас"))
      return action("track.delete", `«${item.title}» трекийг устгана.`, item.id);
    if (item && has(q, "publish", "нийтэл"))
      return action("track.update", `«${item.title}» трекийг published болгоно.`, item.id, {
        status: "published",
      });
  }

  return {
    text: "Командын зорилгыг ойлгосон ч яг гүйцэтгэх объект эсвэл параметр дутуу байна. Нэр/ID болон хийх үйлдлээ тодорхой бичнэ үү.",
    state: "listening",
  };
}
...
async function runCustomAction(action: CopilotAction, actor: AdminActor) {
  if (
    actor.role !== "owner" ||
    actor.uid !== firebaseAuth.currentUser?.uid ||
    !isAdminEmail(firebaseAuth.currentUser?.email)
  ) {
    return {
      ok: false as const,
      error: "Энэ copilot үйлдэлд зөвшөөрөгдсөн OWNER бүртгэл шаардлагатай.",
    };
  }

  try {
    if (action.kind === "gallery.create") {
      const ref = await addDoc(collection(firebaseDb, "gallery"), {
        ...(action.payload ?? {}),
    ...
    if (action.kind === "gallery.update") {
      ...
      await updateDoc(doc(firebaseDb, "gallery", action.targetId), {
        ...(action.payload ?? {}),
        updatedAt: serverTimestamp(),
        updatedBy: actor.uid,
      });
    ...
    await recordAuditEvent({
      ...
      action: action.kind,
      target: action.targetId,
      severity: action.risk === "high" ? "critical" : "warning",
      result: "failure",
      detail: "copilot_backend_failure",
    });
...
export async function executeAdminCopilotAction(action: CopilotAction, actor: AdminActor | null) {
  ...
  if (action.kind.startsWith("gallery.") || action.kind === "meet.delete")
    return runCustomAction(action, actor);
  const result = await dispatchAdminAction(
    { kind: action.kind as AdminActionKind, targetId: action.targetId, payload: action.payload },
    actor,
  );
